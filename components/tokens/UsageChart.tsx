...
export default function UsageChart() {
  ...
  return (
    <div className="bg-[#1E293B] border border-[#334155] rounded-lg overflow-hidden shadow-sm select-none">
      <div className="p-4 sm:p-6 border-b border-[#334155] flex justify-between items-center bg-[#1E293B]">
        ...
        <div className="flex bg-[#0F172A] p-0.5 rounded border border-[#334155] text-xs">
          <button 
            type="button"
            onClick={() => setRange('7d')}
            className={`px-3 py-1 font-bold rounded ${range === '7d' ? 'bg-[#1E293B] text-[#06B6D4] border border-[#334155]' : 'text-[#94A3B8] hover:text-white'}`}
          >
            7d
          </button>
          <button 
            ...
            className={`px-3 py-1 font-bold rounded ${range === '30d' ? 'bg-[#1E293B] text-[#06B6D4] border border-[#334155]' : 'text-[#94A3B8] hover:text-white'}`}
          >
            30d
          </button>
        </div>
      ...
      {/* Chart Grid */}
      <div className="p-6 sm:p-8 h-64 flex items-end justify-between gap-2 sm:gap-4 bg-[#0F172A]/20">
        {days.map((day, idx) => (
          <div key={idx} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end">
            
            {/* Bar shape */}
            <div className="w-full bg-[#0F172A] hover:bg-[#0F172A]/80 h-[80%] relative rounded-t-sm transition-all duration-300 border border-[#334155]/30">
              
              {/* Tooltip */}
              <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-[#1E293B] border border-[#334155] text-white px-2 py-1 rounded text-[9px] font-mono whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 font-bold">
                {day.tokens}
              </div>
