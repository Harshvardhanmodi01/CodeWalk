// Compact yet complete pre-populated questions for 20 topics (10 questions each = 200 questions total)
// This file exports a clean array of questions with category: "technical" and structured JSON expected answers.
...
// Generates rich expected answer objects for all 200 questions to avoid huge file size.
// Returns a complete array of 200 question records.
const getQuestions = () => {
  const list = [];
  
  for (const [topic, qs] of Object.entries(topicsData)) {
    qs.forEach((item, index) => {
      let expectedAnswerObj = {};
      
      // Since all seed questions are technical, use the technical expected answer shape:
      expectedAnswerObj = {
...
const seedQuestions = getQuestions();
