...
exports.generateFinalQuestionsJson = generateFinalQuestionsJson;
...
function getClient() {
    if (!client) {
        ...
        client = new groq_sdk_1.default({ apiKey });
    }
    return client;
}
// ---------- Quality filter ----------
function isLowQualityQuestion(text) {
...
function isValidQuestion(q) {
    ...
    if (!q.answer || typeof q.answer !== 'string')
        return false;
    if (q.answer.length < 10 || q.answer.length > 500)
        return false;
 ...
 */
async function adversarialCritique(questionsArray, code, readmeContext) {
...
Return JSON: 
{
  ...
  "summary": "Brief overall assessment."
    ...
    try {
        ...
        const validIndices = new Set(parsed.validQuestions?.map((v) => v.index) || []);
...
async function selfCritiqueQuestions(questionsArray, code) {
...
function getImportantLinesFromAST(astAnalysis) {
    ...
    if (astAnalysis.functions) {
        for (const fn of astAnalysis.functions) {
            if (fn.startLine)
                important.add(fn.startLine);
            if (fn.endLine)
                important.add(fn.endLine);
        }
    }
    // Conditionals
...
function buildFilePrompt(code, filename, readmeContext, projectName, astAnalysis) {
    ...
    if (astAnalysis && totalLines > MAX_LINES_BEFORE_FILTER) {
        ...
        for (const ln of important) {
            for (let offset = -CONTEXT_LINES_AROUND_IMPORTANT; offset <= CONTEXT_LINES_AROUND_IMPORTANT; offset++) {
                ...
                if (neighbor >= 1 && neighbor <= totalLines)
    ...
    if (astAnalysis && astAnalysis.complexity !== undefined) {
        ...
        if (astAnalysis.conditionals?.length)
            astHints += `- Conditionals: ${astAnalysis.conditionals.length} (use for edge case question)\n`;
        if (astAnalysis.loops?.length)
            astHints += `- Loops: ${astAnalysis.loops.length} (check for off-by-one or infinite loop in adversarial question)\n`;
        if (astAnalysis.asyncCalls?.length)
            astHints += `- Async calls: ${astAnalysis.asyncCalls.map((c) => c.callee).join(', ')} (check error handling for adversarial question)\n`;
    }
    // Add code block
...
// ---------- Groq caller ----------
async function callGroq(systemPrompt, userPrompt, customModel) {
...
// ---------- Validation ----------
async function parseAndValidateResponse(rawResponse, retryCount = 0) {
...
// ---------- Public API ----------
async function generateQuestionsRaw(code, filename, readmeContext, projectName, model) {
    if (!code || !filename)
...
async function generateFinalQuestions(readme, projectName, model, hasReadme = true) {
...
async function generateFinalQuestionsJson(readme, projectName) {
