...
exports.calculateAuthenticityScore = calculateAuthenticityScore;
exports.runRepoAuthenticityCheck = runRepoAuthenticityCheck;
exports.fetchFileCommitTimeline = fetchFileCommitTimeline;
 ...
 */
function calculateAuthenticityScore(commitCount, spanHours, uniqueDays, averageGapMinutes, genericMessageRatio, totalAdditions, largestCommitRatio, aiProbability, aiVerdict) {
    const flags = [];
 ...
 */
async function runRepoAuthenticityCheck(owner, repo, githubToken, combinedCodeSample) {
    const token = githubToken || process.env.GITHUB_TOKEN;
    const groqKey = process.env.GROQ_API_KEY;
    let commitsList = [];
    try {
        const commitRes = await (0, github_1.fetchGitHub)(`https://api.github.com/repos/${owner}/${repo}/commits?per_page=100`, token);
        if (commitRes.ok) {
    ...
    const recentCommitsDetails = [];
    const limit = Math.min(commitCount, 15);
    for (let i = 0; i < limit; i++) {
        const c = commitsList[i];
        try {
            const detailRes = await (0, github_1.fetchGitHub)(`https://api.github.com/repos/${owner}/${repo}/commits/${c.sha}`, token);
            if (detailRes.ok) {
    ...
    // Parse commit dates
    const dates = commitsList.map((c) => new Date(c.commit.author?.date || c.commit.committer?.date).getTime());
    const latest = Math.max(...dates);
    const earliest = Math.min(...dates);
    const spanHours = (latest - earliest) / (1000 * 60 * 60);
    // Get unique calendar days
    const uniqueDays = new Set(commitsList.map((c) => new Date(c.commit.author?.date || c.commit.committer?.date).toISOString().split('T')[0])).size;
    ...
    const messages = commitsList.map((c) => c.commit.message || '');
    ...
    const { score, flags } = calculateAuthenticityScore(commitCount, spanHours, uniqueDays, averageGapMinutes, genericMessageRatio, totalAdditions, largestCommitRatio, aiProbability, aiVerdict);
    return {
        score,
        flags,
        signals: {
 ...
 */
async function fetchFileCommitTimeline(owner, repo, filePath, token) {
    const tokenToUse = token || process.env.GITHUB_TOKEN;
    // 1. Fetch last 30 commits touching the file
    let commits = [];
    try {
        const res = await (0, github_1.fetchGitHub)(`https://api.github.com/repos/${owner}/${repo}/commits?path=${encodeURIComponent(filePath)}&per_page=30`, tokenToUse);
        if (res.ok) {
    ...
    for (const idx of selectedIndices) {
        ...
        try {
            ...
            const detailRes = await (0, github_1.fetchGitHub)(`https://api.github.com/repos/${owner}/${repo}/commits/${c.sha}`, tokenToUse);
            if (detailRes.ok) {
                const detail = await detailRes.json();
                const fileDetail = detail.files?.find((f) => f.filename === filePath);
