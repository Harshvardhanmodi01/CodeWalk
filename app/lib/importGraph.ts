...
/** A repo file together with its already-fetched source text. */
export interface FileWithContent {
  /** Repo-relative path, forward-slash normalised (e.g. "src/utils/helpers.ts"). */
  path: string;
  /** Raw source text of the file. */
  content: string;
}

/** The full import graph produced by buildImportGraph(). */
export interface ImportGraph {
  /**
   * importedBy[filePath] = array of file paths that import this file.
   * Every file in the input set has an entry (possibly empty).
   */
  importedBy: Record<string, string[]>;

  /**
   * imports[filePath] = array of resolved file paths this file imports
   * (only files that exist in the input set; external packages are dropped).
   */
 ...
 */
export function buildImportGraph(files: FileWithContent[]): ImportGraph {
  ...
  for (const file of files) {
    ...
    for (const spec of specifiers) {
      ...
      if (resolved === null) continue; // external or unresolvable
 ...
 */
export function getInDegrees(graph: ImportGraph): Record<string, number> {
 ...
 */
export function getImportCentrality(
 ...
 */
export function getImportCentralityFromGraph(
  graph: ImportGraph
): Record<string, number> {
  const degrees = getInDegrees(graph);
